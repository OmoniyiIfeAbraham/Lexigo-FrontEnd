...
const GeneralQuiz = () => {
  ...
  const handleNext = async () => {
    ...
    try {
      await axios.get(
        `${BaseUrl}/api/general/test/type/progress/add?id=${id}&level=${currentQuestion}&score=${newScore}&Pscore=${newPscore}&Sscore=${newSscore}`,
        {
          headers: {
  ...
  const StartQuiz = async () => {
    ...
    try {
      let url = `${BaseUrl}/api/general/test/type/start?id=${id}`;
  ...
  const handleSubmit = async () => {
    ...
    try {
      await axios.get(
        `${BaseUrl}/api/general/test/type/progress/add?id=${id}&level=${currentQuestion}&score=${finalScore}&Pscore=${newPScore}&Sscore=${newSScore}&result=${encodeURIComponent(
          dyslexiaType
        )}`,
        {
          headers: {
  ...
  return (
    <div className="w-full">
      <NavComp />
      <main className="flex-1 px-6" style={{ backgroundColor: Colors.White }}>
